...
def montar_intervalo_datas(dia_ini_str: str, dia_fim_str: str, mes_ano_str: str) -> Tuple[str, str]:
    """
    Recebe:
      - dia_ini_str: "1", "01", "1.0", etc. (obrigatório)
      - dia_fim_str: "3", "3.0", "", etc. (opcional; se vazio, assume igual ao início)
      - mes_ano_str: "03/26" ou "03/2026"

    Retorna:
      (data_inicio, data_fim) no formato "dd/mm/aaaa"
    """
    # Normaliza dia início
    if not isinstance(dia_ini_str, str):
        dia_ini_str = str(dia_ini_str)
    dia_ini_str = dia_ini_str.strip()
    if not dia_ini_str:
        raise ValueError("Dia_Inicio vazio ou inválido.")

    if "." in dia_ini_str:
        dia_ini = int(float(dia_ini_str))
    else:
        dia_ini = int(dia_ini_str)

    # Normaliza dia fim (pode vir vazio, NaN, 27.0, etc.)
    if not isinstance(dia_fim_str, str):
        dia_fim_str = str(dia_fim_str)
    dia_fim_str = dia_fim_str.strip()

    if not dia_fim_str or dia_fim_str.lower() == "nan":
        dia_fim = dia_ini
    else:
        if "." in dia_fim_str:
            dia_fim = int(float(dia_fim_str))
        else:
            dia_fim = int(dia_fim_str)

    # Normaliza Mes/Ano
    if not isinstance(mes_ano_str, str):
        mes_ano_str = str(mes_ano_str)
    mes_ano_str = mes_ano_str.strip()
    if not mes_ano_str:
        raise ValueError("Mes/Ano vazio ou inválido.")

    try:
        mes_str, ano_str = mes_ano_str.split("/")
    except ValueError:
        raise ValueError(f"Mes/Ano em formato inválido: '{mes_ano_str}' (esperado mm/aa ou mm/aaaa).")
    ...
    data_ini = datetime.date(ano, mes, dia_ini)
    data_fim = datetime.date(ano, mes, dia_fim)

    if data_fim < data_ini:
        raise ValueError(
            f"Intervalo de dias inválido: Dia_Fim ({dia_fim}) < Dia_Inicio ({dia_ini}) no mês/ano {mes_ano_str}."
        )

    data_ini_fmt = data_ini.strftime("%d/%m/%Y")
    data_fim_fmt = data_fim.strftime("%d/%m/%Y")
    return data_ini_fmt, data_fim_fmt
...
def carregar_justificativas(caminho_csv: str) -> pd.DataFrame:
    """
    Carrega o CSV de justificativas no novo formato:
    ...
    if "Dia Inicio" in df.columns and "Dia_Inicio" not in df.columns:
        rename_map["Dia Inicio"] = "Dia_Inicio"
    if "Dia Fim" in df.columns and "Dia_Fim" not in df.columns:
        rename_map["Dia Fim"] = "Dia_Fim"
    if "Mes_Ano" in df.columns and "Mes/Ano" not in df.columns:
        rename_map["Mes_Ano"] = "Mes/Ano"
    if rename_map:
        df = df.rename(columns=rename_map)

    colunas_esperadas = ["Status", "Nome_Completo", "Dia_Inicio", "Dia_Fim", "Mes/Ano", "Motivo"]
    ...
    # Gera Data_Inicio e Data_Fim a partir de Dia_Inicio / Dia_Fim / Mes/Ano
    datas_inicio = []
    datas_fim = []

    for _, row in df.iterrows():
        dia_ini_raw = row["Dia_Inicio"]
        dia_fim_raw = row["Dia_Fim"]
        mes_ano_raw = row["Mes/Ano"]

        dia_ini_str = "" if pd.isna(dia_ini_raw) else str(dia_ini_raw)
        dia_fim_str = "" if pd.isna(dia_fim_raw) else str(dia_fim_raw)
        mes_ano_str = "" if pd.isna(mes_ano_raw) else str(mes_ano_raw)

        data_ini_fmt, data_fim_fmt = montar_intervalo_datas(dia_ini_str, dia_fim_str, mes_ano_str)
        datas_inicio.append(data_ini_fmt)
        datas_fim.append(data_fim_fmt)

    df["Data_Inicio"] = datas_inicio
    df["Data_Fim"] = datas_fim

    # Reorganiza colunas para o resto do código continuar igual
    df = df[["Nome_Completo", "Motivo", "Data_Inicio", "Data_Fim", "Status"]]
...
def preparar_df_para_salvar(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        data_ini = parse_data_br(str(row["Data_Inicio"]))
        data_fim = parse_data_br(str(row["Data_Fim"]))
        ...
        dias_ini.append(data_ini.day)
        dias_fim.append(data_fim.day)
        meses_ano.append(data_ini.strftime("%m/%Y"))

    df_novo = pd.DataFrame({
        "Status": df["Status"].fillna(""),
        "Nome_Completo": df["Nome_Completo"],
        "Dia_Inicio": dias_ini,
        "Dia_Fim": dias_fim,
        "Mes/Ano": meses_ano,
        "Motivo": df["Motivo"],
...
def garantir_tela_lista(page: Page):
    """
    Garante que a tela de lista está carregada.

    Se não conseguir, registra erro e pede intervenção manual:
    você volta para a LISTA e aperta ENTER.
    """
    ...
    try:
        ...
        print("Volte manualmente para a LISTA de funcionários (campo 'Pesquisar...' e tabela).")
...
def voltar_da_tela_espelho_para_lista(page: Page) -> None:
    ...
    Tenta, nesta ordem:
    - #btn-buscar-crud (principal)
    - botão 'Buscar' visível dentro de div.col-md-2.mt-2
    - botão 'Voltar' visível na área de conteúdo

    Se nada funcionar, registra erro e pede intervenção manual.
    """
    ...
    try:
        ...
        candidatos.append(page.locator("#btns-broadcast #btn-buscar-crud"))

        candidatos.append(
            page.locator("div.col-md-2.mt-2 button.btn.btn-primary.btn-round.ml-auto")
        )
    ...
    with sync_playwright() as p:
        ...
        for idx in range(len(df)):
            ...
            if nome_proximo_norm is None or nome_proximo_norm != nome_atual_norm:
                ...
                df_para_salvar = preparar_df_para_salvar(df)
